...
def test_discover_cisco_prime_wifi_connections() -> None:
...
def test_check_cisco_prime_wifi_connections_no_levels() -> None:
    """Test check function for cisco_prime_wifi_connections check without configured levels."""
    parsed = parse_cisco_prime_wifi_connections(_STRING_TABLE)
...
def test_check_cisco_prime_wifi_connections_with_lower_levels() -> None:
    """Test check function for cisco_prime_wifi_connections check with configured lower levels."""
    parsed = parse_cisco_prime_wifi_connections(_STRING_TABLE)
    result = list(check_cisco_prime_wifi_connections({"levels_lower": (20000, 10000)}, parsed))
    assert result == [
        Result(state=State.CRIT, summary="Total connections: 7247 (warn/crit below 20000/10000)"),
        Metric("wifi_connection_total", 7247),
        Result(state=State.OK, summary="802.11a: 1227"),
        Metric("wifi_connection_dot11a", 1227),
        Result(state=State.OK, summary="802.11b: 0"),
