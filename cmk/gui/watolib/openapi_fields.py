...
from cmk.fields import base, Boolean, Constant, Integer, List, Nested, String, Time
...
from marshmallow.warnings import RemovedInMarshmallow4Warning  # noqa: E402
...
def from_timestamp(value: float) -> _datetime:
    return _datetime.fromtimestamp(value, tz=UTC)


def to_timestamp(value: _datetime) -> float:
    return float(_datetime.timestamp(value))
...
class Timestamp(DateTime):
    ...
    def _deserialize(  # type: ignore[override]
        ...
        return _datetime.timestamp(val)
...
def verify_group_exists(group_type: GroupType, name: GroupName) -> bool:
    return name in load_group_information().get(group_type, {})
