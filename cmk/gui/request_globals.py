...
def _get_output_format(output_format: str) -> str:
    if output_format not in _OUTPUT_FORMAT_MIME_TYPES:
        return "html"
    return output_format


def _get_mime_type_from_output_format(output_format: str) -> str:
...
def set_global_vars() -> None:
    ...
    output_format = _get_output_format(request.args.get("output_format", default="html", type=str))
    response = cast(http.Response, current_app.make_response(""))
    response.mimetype = _get_mime_type_from_output_format(output_format)
